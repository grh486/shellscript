...
read -t 60 -p "请输入elasticsearch集群节点1的IP：" cluster_IP1
echo -e "\n"
echo "elasticsearch节点1的IP为：$cluster_IP1"
read -t 60 -p "请输入elasticsearch集群节点3的IP：" cluster_IP3
echo -e "\n"
echo "elasticsearch节点3的IP为：$cluster_IP3"
echo "discovery.zen.ping.unicast.hosts: ["$cluster_IP1","$ip", "$cluster_IP3"]" >> /opt/elasticsearch/config/elasticsearch.yml
